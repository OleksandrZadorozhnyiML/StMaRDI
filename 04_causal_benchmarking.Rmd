...
for (i in setdiff(whitelist,drop_f)) {
  # Ground truth files
  if (i %in% c(86, 88)) {
    expected = c(1, 2)
  } else {
    ...
    description = paste(readLines(description_file), collapse=" ")
    description = gsub(" ", "", description)
    if (grepl(">[Yy]|[Yy]<", description)) {
      expected = c(1, 2)
    } else if (grepl(">[Xx]|[Xx]<", description)) {
      expected = c(2, 1)
    } else {
  ...
  data_file <- url(sprintf("https://webdav.tuebingen.mpg.de/cause-effect/pair%04d.txt", i))
    ...
    if (hsic_12$p.value<p_val){
      result = c(1,2)
    }
    else if (hsic_21$p.value<p_val){
      result = c(2,1)
    }
    else {
      result = c(1,1)
