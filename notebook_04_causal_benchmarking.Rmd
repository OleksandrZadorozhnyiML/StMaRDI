...
Note that not always the first column is the cause and the second the effect. This is indicated in a meta-data file. 


***Setup of the problem***
We have 89 datasets with variables $Var1, Var2$. The goal is to use HSIC test to establish (empirically) the relationship between the effect and cause in the datasets while using non-linear testing mechanism. 
```{r}
### loading packages
library("liver")
library("qgraph")
...
print(selected)
```
Here we taking a glimpse in the Tuebingen dataset collection
...
We are considering the datasets which have 2 variables: in the following we use the HSIC [1,4] criterion to test for independence of the residuals and covariates
```{r}
library("utils")
...
[4] Peters, J., Mooij, J.M., Janzing, D. and Schölkopf, B., 2014. Causal discovery with continuous additive noise models.
